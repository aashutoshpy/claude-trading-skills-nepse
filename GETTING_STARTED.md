...
| `CERTIFICATE_VERIFY_FAILED: unable to get local issuer certificate` when hitting `nepalstock.com.np` | `nepalstock.com.np` presents an incomplete TLS cert chain (missing intermediate) — not a local trust-store issue; certifi can't fix it either | Use `--symbols NABIL UPPER NICA` style invocations for the names you actually want (the VCP screener falls back to symbol stubs if it can't fetch the universe). For full-universe runs, switch to the community backend: `pip install nepse-api && NEPSE_BACKEND=community python3 ...` |
